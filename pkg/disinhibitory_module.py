...
class Disinhibitory_module(object):
    def __init__(
        self,
        params,
    ):
        ...
        self.decay = 0.9
        self.tau = params["tau"]
        self.wta_speed = params["wta_speed"]
        self.lr_msp = params["lr_msp"]

        # WTA parameters
        self.external_current = params["external_current"]
        self.beta = torch.FloatTensor([params["beta"]]).to(device)
        self.r = params["r"]
        self.self_inhib = params["self_inhib"]
        self.module_inhib = params["module_inhib"]
        ...
        self.FF_scaling = self.input_shape / 16
        (
            self.FF_msp_weights,
            self.FB_msp_weights,
            self.WTA_matrix,
    ...
    def initiate_msp_weights(self):
        ...
        FF_msp_weights /= 10 * self.FF_scaling
    ...
    def wta_update_module(self, x, msp_spikes, wta_input):
        """
        ...
        step = self.wta_speed * (
            ...
            + torch.Tensor(x.shape).normal_(1, .1)  # self.external_current
        )
    ...
    def update_layer(self, input_potential, EPSC_decay, refractoriness):
        ...
        idx = self.phi(membrane_potential) > torch.Tensor(
            self.batch_size, self.msp_neurons * self.n_memory).uniform_()
        spikes[idx] = 1.0
    ...
    def forward(self, input_, commitement_modulation, learning=True):
        ...
        self.active_memory_activity += (1 / 10 * (-self.active_memory_activity + torch.sum(
            self.inhibitory_msp_spikes.reshape(self.batch_size, self.n_memory, -1), axis=2))).detach()

        if learning:
            commitement_modulation = commitement_modulation.repeat_interleave(
                self.msp_neurons, 1).detach()
            ...
            self.commitment_matrix[self.commitment_matrix > 0.14] = 0.14 # move 1
# self.commitment_matrix[self.commitment_matrix >0.085] = 0.085 ## move 2
            self.commitment_matrix.detach()
        self.memory_output = torch.einsum(
            "bjk,bk->bj",
            self.FB_msp_weights,
            EPSC_inhibitory_msp).detach()
        self.memory_output = torch.unsqueeze(
            self.memory_output, 1).repeat_interleave(
            2, dim=1).detach()
    ...
    def plot_network(self):
        ...
        if self.plot:
            ...
            plt.imshow(self.commitment_matrix.cpu()[0].detach())
            plt.colorbar()
            plt.show()
